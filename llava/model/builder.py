...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", torch_dtype="float16",attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if model_base is not None:
            ...
            tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
            model = AutoModelForCausalLM.from_pretrained(model_base, torch_dtype=torch.float16, low_cpu_mem_usage=True, device_map=device_map)
            print(f"Loading LoRA weights from {model_path}")
            non_lora_able = torch.load(f"{model_path}/non_lora_trainables.bin")
            new_weight = {}
            for key,values in non_lora_able.items():
                new_name = key.replace('base_model.model.','')
                # non_lora_able[new_name] = values
                new_weight[new_name] = values
            msg=model.load_state_dict(new_weight,strict=False)
            model = PeftModel.from_pretrained(model, model_path)
            print(f"Merging weights")
            # non_lora_able = torch.load("/kernel/cv/dcsi/coding/mmsegmentation-main/data/Space_Net/non_lora_trainables.bin")
            model = model.merge_and_unload()
